...
;             se puede obtener un número pseudoaleatoreo.
(define (seed valor)
...
;Tipo 1: Formal.
;Recorrido:
;Se devuelve una lista en donde el primer elemento de esta es el tipo
;de chatbot, el resto equivalen a la acumulación de las evaluaciones que tenga el chatbot.
;Descripción: Se crea una lista de enteros en donde el primer entero identifica la personalidad del bot y toma las evaluaciones de algún chat anterior para añadirlas al actual.
;CONSTRUCTOR
(define (chatBot personalidad anteriorListaChatbot)
     ...
     (cond
          ...
          (else (append listaChatbot '(1) (separarEvaluaciones anteriorListaChatbot)))
          )
)
...
(define (sendMessage msg chatBot log seed)
     (if (or (eqv? msg "¿Qué pasó con el stock?") (eqv? msg "¿Que paso con el stock?"))
          ;do
     ...
     (cond
          ((eqv? (adquirirUltimo log) "|Nombre|")
               (if (= (adquirirPersonalidad chatBot) 1) ;Personalidad: Formal.
                    ;do
                    (cond
                         ((= seed 0) (append log (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¿Cómo estás" msg "?" "|Respuesta1|")))
          ...
          ((eqv? (adquirirUltimo log) "|Respuesta1|")
               (append log (respuesta1 msg chatBot seed)(list "Estos son las marcas/modelos de autos de las homocinéticas que disponemos: 1.-Toyota Rav4, 2.-Renault Duster, 3.-Hyundai Tucson, 4.-Nissan Qashqai, 5.-Nissan Kicks, elija la opción que desea." "|Respuesta2|"))
               )
          ((eqv? (adquirirUltimo log) "|Respuesta2|")
               (append log (respuesta2 msg log) (list "Perfecto, ¿Cuántas querrá?" "|Respuesta3|"))
               )
...
(define (respuesta1 msg chatBot seed)
     (cond
          ((or (eqv? msg "Bien") (eqv? msg "bien"))
               (if (= (adquirirPersonalidad chatBot) 1) ;Personalidad: Formal.
                    ;do
                    (cond
                         ((= seed 0) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Oh ¡Perfecto!")))
                         ((= seed 1) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡Que bien!")))
                         )
                    ;else Personalidad: Informal.
                    (cond
                         ((= seed 0) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡Buena!" )))
                         ((= seed 1) (append (list "Usuario:" msg ) (Hora&Fecha current-date '()) (list "Chatbot:" "¡Buena po!")))
                         )
                    )
               )
          ((or(eqv? msg "Mal") (eqv? msg "mal"))
               (if (= (adquirirPersonalidad chatBot) 1) ;Personalidad: Formal.
                    ;do
                    (cond
                         ((= seed 0) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Oh ¡Que mal, espero poder ayudarte!")))
                         ((= seed 1) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Espero poder subirte el ánimo cuando lleves lo que buscabas.")))
                         )
                    ;else Personalidad: Informal.
                    (cond
                         ((= seed 0) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡Oh que mala!" )))
                         ((= seed 1) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡de perro!")))
                         )
                    )
               )
          (else (append (list "Usuario:" msg ) (list "Chatbot:" "Lo lamento, no logro entenderte, ¿Podrías repetirlo?" "|Respuesta1|")))
     )
)

(define (respuesta2 msg log)
     (cond
          ((eqv? msg "1") (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Tomo nota...")))
          ((eqv? msg "2") (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Tomo nota...")))
          ((eqv? msg "3") (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Tomo nota...")))
          ((eqv? msg "4") (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Tomo nota...")))
          ((eqv? msg "5") (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "Tomo nota...")))
          (else (append (list "Usuario:" msg ) (list "Chatbot:" "Lo lamento, no logro entenderte, ¿Podrías repetirlo?" "|Respuesta2|")))
          )
)
...
(define (respuesta4 msg)
     (cond
          ((or(eqv? msg "Sí") (eqv? msg "Si"))(append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡Okey!" "Estos son las marcas/modelos de autos de las homocinéticas que disponemos: 1.-Toyota Rav4, 2.-Renault Duster, 3.-Hyundai Tucson, 4.-Nissan Qashqai, 5.-Nissan Kicks, elija la opción que desea." "|Respuesta2|")))
          ((or(eqv? msg "Sí, me gustaría comprar algo más") (eqv? msg "Si, me gustaria comprar algo más")) (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡Okey!" "Estos son las marcas/modelos de autos de las homocinéticas que disponemos: 1.-Toyota Rav4, 2.-Renault Duster, 3.-Hyundai Tucson, 4.-Nissan Qashqai, 5.-Nissan Kicks, elija la opción que desea." "|Respuesta2|")))
          ((eqv? msg "No, gracias") (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡Está bien! Todo el stock se ha confirmado y todos sus items han sido agregados a su carro.")))
          ((eqv? msg "No") (append (list "Usuario:" msg ) (Hora&Fecha current-date) (list "Chatbot:" "¡Está bien! Todo el stock se ha confirmado y todos sus items han sido agregados a su carro.")))
          (else (append (list "Chatbot:" "Lo lamento, no logro entenderte, ¿Podrías repetirlo?" "|Respuesta4|")))
          )
)
...
#|
LLamada general:
     (define cb1 (chatBot "formal" cb))

     (define log1 (begingDialog cb1 log (seed 1)))

     (define log2 (sendMessage "Omar Carrasco" cb1 log1 (seed 1)))

     (define log3 (sendMessage "bien" cb1 log2 (seed 1)))

     (define log7 (endDialog cb1 log6 (seed 1)))
